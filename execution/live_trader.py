...
def _notify_discord(message: str, color: int = 0x3498db, title: str = "Ruby Update", destination: str = "scans") -> None:
    ...
    try:
        payload = {
            "embeds": [{
                ...
                "description": message,
                "color": color,
                "footer": {"text": "Ruby Trading Systems | v2.0"},
...
def run_live_trading_loop(exchange_id: str, symbol: str):
    ...
    _notify_discord("💎 Ruby is live in Public/Paper mode. No API Keys required.", 
                    color=0x9b59b6, title="SYSTEM ONLINE", destination="status")

    # Connect Anonymously
    exchange = create_exchange(exchange_id, os.getenv("BINANCE_API_KEY"), os.getenv("BINANCE_API_SECRET"))
    
    strategy = MeanReversionBollingerStrategy(
        MeanReversionConfig(rsi_oversold=25.0, rsi_overbought=75.0, bb_std=2.25, ema_trend_period=200)
    )

    safety = SafetyModule(SafetyConfig(discord_webhook_url=DISCORD_ALERTS_WEBHOOK_URL))

    last_bar_time, last_position, last_heartbeat = None, 0, datetime.now(timezone.utc)

    print(f"🚀 Ruby Initialized (Public Mode) | Exchange: {exchange_id} | Symbol: {symbol}")
    ...
    while True:
        try:
            ...
            _notify_discord(discord_dash, color=0x3498db, title=f"📡 {current_dt} SCAN", destination="scans")
            ...
            if now_utc - last_heartbeat >= timedelta(hours=4):
                _notify_discord(f"Ruby is alive and tracking {symbol} anonymously.", 
                                color=0x9b59b6, title="❤️ HEARTBEAT", destination="status")
            ...
            if signal != 0 and position != last_position:
                ...
                _notify_discord(sig_msg, color=0x2ecc71 if signal == 1 else 0xe74c3c, 
                                title="💎 TRADE SIGNAL", destination="alerts")
                
                with open(PAPER_LOG_FILE, "a") as f:
                    f.write(f"{datetime.now(timezone.utc).isoformat()} - PUBLIC PAPER - {side.upper()} @ {curr_price}\n")
...
def main():
    run_live_trading_loop(
        os.getenv("EXCHANGE_ID", "binance"), # Switched to binance global for better public access
